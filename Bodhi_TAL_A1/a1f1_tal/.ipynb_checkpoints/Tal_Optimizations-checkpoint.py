...
def createNewPar(seq_change, count):
    new_filename = f"a1f1_shift{count}"
    header = a1f1[0:3].copy() # constant header that each par file must have
    starting_frozen = a1f1[3:17].copy() #first 14 frozen
    new_model = a1f1[17:136].copy() # the parameter data we want to edit (taking out the first 14 and last 14)
    ending_frozen = a1f1[136: 150].copy() #last 14 frozen
